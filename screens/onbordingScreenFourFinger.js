...
export const OnbordingFourFinger  = ({ navigation, route }) => {
    return (
        <SafeAreaView style={{flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: '#101010'}}>
            <NavbarContainer>
                <ButtonBack onPress={() => navigation.goBack()} />
                <Numerator title='6' title_sec='17'/>
            </NavbarContainer>
            <RoomName>Комната №1</RoomName>
            <Field>
            <TextField>
                <Text style={{fontSize: 15, lineHeight: 25, color: 'white'}}>Прикоснитесь к выделенной области 4 пальцами</Text>
                <FingerValue>
                    <Text style={{fontSize: 12, lineHeight: 20, color: 'white'}}>печаль, тревога, тоска, обида, гнев</Text>
                </FingerValue>
            </TextField>
            <GestureField>
                <Image source={require('../assets/fourFingerAnimation.png')} style={{width: 295, height: 263}}/>
            </GestureField>
            </Field>
            <ButtonPrimary onPress={() => navigation.navigate('Onbording_9')} title='Дальше' />
        </SafeAreaView> 
    );
  }
