...
export const OnbordingThreeFinger  = ({ navigation, route }) => {
    return (
        <SafeAreaView style={{flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: '#101010'}}>
            <NavbarContainer>
                <ButtonBack onPress={() => navigation.goBack()} />
                <Numerator title='5' title_sec='17'/>
            </NavbarContainer>
            <RoomName>Комната №1</RoomName>
            <Field>
            <TextField>
                <Text style={{fontSize: 15, lineHeight: 25, color: 'white'}}>Прикоснитесь к выделенной области 3 пальцами</Text>
                <FingerValue>
                    <Text style={{fontSize: 12, lineHeight: 20, color: 'white'}}>нежность, забота, поддержка</Text>
                </FingerValue>
            </TextField>
            <GestureField>
                <Image source={require('../assets/threeFingerAnimation.png')} style={{width: 259, height: 219}}/>
            </GestureField>
            </Field>
            <ButtonPrimary onPress={() => navigation.navigate('Onbording_8')} title='Дальше' />
        </SafeAreaView> 
    );
  }
