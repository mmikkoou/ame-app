...
export const OnbordingTouchGesture  = ({ navigation, route }) => {
    return (
        <SafeAreaView style={{flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: '#101010'}}>
            <NavbarContainer>
                <ButtonBack onPress={() => navigation.goBack()} />
                <Numerator title='9' title_sec='17'/>
            </NavbarContainer>
            <RoomName>Комната №1</RoomName>
            <Field>
            <TextField>
                <Text style={{fontSize: 15, lineHeight: 25, color: 'white'}}>Прикоснитесь к экрану двумя пальцами</Text>
            </TextField>
            <GestureField>
                <Image source={require('../assets/twoFingerAnimation.png')} style={{width: 219, height: 219}}/>
            </GestureField>
            </Field>
            <ButtonPrimary onPress={() => navigation.navigate('Onbording_12')} title='Дальше' />
        </SafeAreaView> 
    );
  }
