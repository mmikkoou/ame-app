...
export const OnbordingZoomGesture  = ({ navigation, route }) => {
    return (
        <SafeAreaView style={{flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: '#101010'}}>
            <NavbarContainer>
                <ButtonBack onPress={() => navigation.goBack()} />
                <Numerator title='11' title_sec='17'/>
            </NavbarContainer>
            <RoomName>Комната №1</RoomName>
            <Field>
            <TextField>
                <Text style={{fontSize: 15, lineHeight: 25, color: 'white'}}>Отлично! Попробуйте сделать зум двумя пальцами</Text>
            </TextField>
            <GestureField>
                <Image source={require('../assets/zoomGestureAnimation.png')} style={{width: 286, height: 286}}/>
            </GestureField>
            </Field>
            <ButtonPrimary onPress={() => navigation.navigate('Onbording_14')} title='Дальше' />
        </SafeAreaView> 
    );
  }
